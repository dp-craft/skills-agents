...
- Touch targets minimum 44×44px (WCAG) / 48×48px (Material)

### Responsive Design

- Test layouts at 320px, 768px, 1024px, 1440px breakpoints
- No horizontal scroll at any standard viewport width
- Font sizes in `rem`/`em`, not `px` (respects user preferences)
...
- Skeleton screens over spinners for content-heavy layouts
- Error boundaries around independent UI sections
- Retry affordance on failed network requests
...
- Never interpolate user input into `innerHTML` or equivalent

### Keyboard

- Arrow keys navigate lists/menus/tabs
- Don't override browser-native shortcuts (Ctrl+F, Ctrl+C, etc.)
- Allow text selection on readable content
- Copy buttons on code blocks, URLs, tokens
...
- `z-index` without a defined scale/tokens
